# University Library Portal — project overview

> Former root `README.md`, moved here when the root README was replaced with a rebuild-postponed status note.
...
Build order and exit criteria: [`build-guide.md`](build-guide.md).
...
Details: [`data-provenance-and-ingestion_v2.md`](data-provenance-and-ingestion_v2.md).
...
**Honest tradeoff:** Prisma has no table inheritance. The six-table resource hierarchy is hand-modeled (shared PK 1:1), with the create invariant owned by a service transaction and exhaustiveness enforced by a TypeScript discriminated union. That cost is intentional — see [`stack-decision_v2.md`](stack-decision_v2.md).
...
```
├── README.md                 ← root status (rebuild postponed)
├── backend/                  ← NestJS API + HTML portal
│   ├── prisma/
│   ├── src/
│   └── test/
└── docs/                     ← design docs; this file is the former root README
```
...
| Doc | What it covers |
|---|---|
| [`build-guide.md`](build-guide.md) | Phased build order and exit criteria — **start here to implement** |
| [`data-provenance-and-ingestion_v2.md`](data-provenance-and-ingestion_v2.md) | Own vs route; what the system fundamentally *is* |
| [`project-structure_v3.md`](project-structure_v3.md) | Modules, API shape, schema |
| [`entity-reference_v2.md`](entity-reference_v2.md) | Entities and relationships in prose |
| [`stack-decision_v2.md`](stack-decision_v2.md) | Why NestJS / Prisma / Postgres / Handlebars+HTMX |
| [`search-design.md`](search-design.md) | Retrieval across dissimilar types; what search deliberately skips |
| [`search-interface-contract.md`](search-interface-contract.md) | Exact `UnifiedSearchService` contract |
| [`deployment-blueprint.md`](deployment-blueprint.md) | Production deploy / CI reasoning |
| [`aws-cloud-infrastructure-plan.md`](aws-cloud-infrastructure-plan.md) | Concrete AWS layout (proposed, not provisioned) |
